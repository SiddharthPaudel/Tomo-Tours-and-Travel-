...
import { 
  Waves, Clock, Zap, ArrowRight, Droplets, Flame, Anchor, LayoutGrid, Info,
  ChevronLeft, CheckCircle, XCircle, Shield, LifeBuoy, Map, X, Loader2, HeartPulse, UserCheck, Phone,CalendarDays, Users
} from 'lucide-react';
...
const RaftingActivity = () => {
  ...
  // 3. BOOKING LOGIC WITH VALIDATION
  const handleFinalBooking = async () => {
    // Validation: Exactly 10 digits and starting with 9
    ...
    return (
      <div className="min-h-screen bg-white font-montserrat pb-20 animate-in fade-in duration-500">
        <AlertModal 
          ...
          onConfirm={() => { setAlertConfig({ ...alertConfig, show: false }); if (alertConfig.type === 'success') setSelectedTrip(null); }}
          {...alertConfig} 
        />
              ...
              <div className="space-y-4">
                <div className="space-y-1">
                  ...
                  <input 
                    ...
                    className="w-full p-5 bg-slate-50 rounded-2xl text-xs font-bold outline-none border-2 border-transparent focus:border-blue-500/20" 
        ...
        <div className="relative h-[65vh] w-full">
          <button onClick={() => setSelectedTrip(null)} className="absolute top-8 left-8 z-20 bg-white/20 backdrop-blur-md p-3 rounded-full text-white hover:bg-white hover:text-blue-900 shadow-xl transition-all"><ChevronLeft size={24} /></button>
      ...
      <section className="max-w-7xl mx-auto px-6 py-24">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
          {allTrips.map((trip) => (
            <div 
              key={trip.id} 
              className="group relative bg-slate-900 rounded-[3rem] overflow-hidden cursor-pointer h-[550px] shadow-2xl transition-all duration-700 hover:-translate-y-3"
            ...
            >
              ...
              <div className="absolute inset-0 p-12 flex flex-col justify-end">
                <div className="translate-y-6 group-hover:translate-y-0 transition-all duration-500">
                  <span className="bg-blue-600 text-white px-5 py-1.5 rounded-full text-[9px] font-black uppercase tracking-[0.2em] mb-5 inline-block shadow-xl">
                    {trip.tag}
                  </span>
                  ...
                  <div className="flex gap-6 pt-6 border-t border-white/10 opacity-0 group-hover:opacity-100 transition-all duration-700 delay-100">
                    ...
                    <div className="flex items-center gap-2 text-blue-400">
                      <Zap size={14}/>
                      <span className="text-[10px] font-black uppercase tracking-widest">{trip.difficulty}</span>
                    </div>
                  </div>
                </div>

                <div className="absolute top-10 right-10 bg-white/10 backdrop-blur-md p-5 rounded-full text-white opacity-0 group-hover:opacity-100 transition-all duration-500 scale-50 group-hover:scale-100 border border-white/20">
                  <ArrowRight size={20} />
                </div>
              </div>
            </div>
        ...
        {allTrips.length === 0 && !loading && (
          <div className="text-center py-20">
            ...
            <p className="text-[10px] font-black uppercase text-slate-400 tracking-[0.3em]">No river expeditions found</p>
          </div>
        )}
      </section>
    </div>
